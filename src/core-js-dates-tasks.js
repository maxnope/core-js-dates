...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const includeFirstDay = 1;
  return msDiff / 1000 / 60 / 60 / 24 + includeFirstDay;
}
...
function isDateInPeriod(date, period) {
  const dateStart = Date.parse(new Date(period.start));
  const dateEnd = Date.parse(new Date(period.end));

  return Date.parse(date) >= dateStart && Date.parse(date) <= dateEnd;
}
/**
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (dateObject.getMonth() === month - 1) {
    ...
    dateObject.setDate(currentDay + 1);
  }
  return weekdays;
}
...
function getWeekNumberByDate(date) {
  ...
  const daysFromStart = Math.floor(
    (dateCopy - startOfYear) / (1000 * 60 * 60 * 24)
  );
  return Math.ceil((daysFromStart + 1) / 7);
}
